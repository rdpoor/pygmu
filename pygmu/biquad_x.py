...
"""
Thoughts on the inner loop:

The inner loop computes:

    y0 = (b0*x0 + b1*x1 + b2*x2) - (a1*y1 + a2*y2)

With a rewrite, y0 can be computed as a dot product:

    y0 = np.dot([b0, b1, b2, -a1, -a2], [x0, x1, x2, y1, y2])

or more generally:

    y0 = np.dot(state, coeffs)

where state is an array with dimensions [channel_count, 5] and
coeffs is a 1-D array of length 5.

Here's the trick.  After each y0 is computed, we need to update the state array.
Rolling the state array and replacing two elements does the job:

    for i in np.arange(n_frames):
        # state = [x1, x2, y1, y2, y3] from previous step
        x0 = X[0, i]
        self._state[4] = x0
        # state = [x1, x2, y1, y2, x0]
        self._state = np.roll(self._state, 1)
        # state = [x0, x1, x2, y1, y2]
        y0 = np.dot(self._state, self._coeffs)
        Y[0, i] = y0
        self._state[2] = y0
        # state = [x0, x1, y0, y1, y2]
"""
# A0 = 0  -- always assumed to be 1.0
# TODO: consider making a coefficient object
A1 = 0
A2 = 1
B0 = 2
B1 = 3
...
# TODO: consider making a previous-state object
X1 = 0
X2 = 1
Y1 = 2
Y2 = 3
