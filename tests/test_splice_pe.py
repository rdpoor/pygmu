...
import unittest
import numpy as np
from pygmu import (ConstPE, CropPE, SplicePE, Extent, PygPE)

class TestSplicePE(unittest.TestCase):
    ...
    def test_init(self):
        ...
        pe = SplicePE(src_pe, up_dur=4, dn_dur=5)
        self.assertIsInstance(pe, SplicePE)
    ...
    def test_render1(self):
        ...
        """
        src_pe = CropPE(ConstPE(10), Extent(0, 10))
        pe1 = SplicePE(src_pe, up_dur=4, dn_dur=5)
        ...
        e = Extent(4, 6)       # fully on for the duration (no ramping)
        expect = np.array([[10, 10]], dtype=np.float32)
        got = pe1.render(e)
        np.testing.assert_array_almost_equal(got, expect)
    ...
    def test_render2(self):
        ...
        self.pe1 = SplicePE(src_pe, up_dur=4, dn_dur=5)
    ...
    def test_extent(self):
        # SplicePE inherits extent rate from source.
        src_pe = CropPE(ConstPE(10), Extent(0, 10))
        pe = SplicePE(src_pe, up_dur=4, dn_dur=5)
        self.assertTrue(src_pe.extent().equals(pe.extent()))

    def test_frame_rate(self):
        # SplicePE inherits frame_rate rate from source.
        src_pe = CropPE(ConstPE(10), Extent(0, 10))
        pe = SplicePE(src_pe, up_dur=4, dn_dur=5)
        self.assertEqual(src_pe.frame_rate(), pe.frame_rate())

    def test_channel_count(self):
        # SplicePE inherits channel_count from source.
        src_pe = CropPE(ConstPE(10), Extent(0, 10))
        pe = SplicePE(src_pe, up_dur=4, dn_dur=5)
